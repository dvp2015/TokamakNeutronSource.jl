...
using XLSX, DataFrames, GLMakie, Interpolations, LinearAlgebra

# ╔═╡ d5bafe4d-7061-4e06-9e4a-e7e9ba9b5824
begin
path = expanduser("~/.julia/dev/TokamakNeutronSource/test/data/TRT_215_8T_NBI.xlsx")
@assert isfile(path)
md"""
...
df = let
	...
	pushfirst!(d, [0.0, d[1,Not(1)]...])  # Fill zero, and keep
	select!(d, :ψ, :T, :n => (x -> 1e13x) => :n)
end
...
T = interpolate(df.ψ, df.T, SteffenMonotonicInterpolation());

# ╔═╡ c8b705bb-e057-4532-bad7-295adbf3823c
n = interpolate(df.ψ, df.n, SteffenMonotonicInterpolation());
...
let
	...
	axt = Axis(
		...
		# aspect=DataAspect(),
		# title=L"$T_i(ψ)$ fit",
		yticks = range(0, 25, length=6),
		yminorticksvisible  = true,
		yminorticks = IntervalsBetween(5),
	)	
	ylims!(axt, 0.0, 25)
	scatter!(axt, df.ψ, df.T, label=L"$T_i$ from table")
	lines!(axt, df.ψ, T.(df.ψ), label=L"$$Interpolation", color=:red, linewidth=2)
	axislegend(axt)	
	hidexdecorations!(axt, grid = false)
	ax2 = Axis(
		f[2,1]; 
		xlabel=L"ψ", 
		ylabel=L"$n_i$, $10^{13} cm^{-3}$", 
		# aspect=DataAspect(),
		# title=L"$n_i(ψ)$ fit",
		yticks = range(0, 10, length=6),
		yminorticksvisible  = true,
		yminorticks = IntervalsBetween(5),
	)	
	ylims!(ax2, 0.0, 10)
	scatter!(ax2, df.ψ, 1e-13.*df.n, label=L"$n_i$ from table")
	lines!(ax2, df.ψ, 1e-13.*n.(df.ψ), label=L"$$Interpolation", color=:red, linewidth=2)
	axislegend(ax2)
...
manifest_format = "2.0"
project_hash = "3dd712f5c55d0c6179a0629b764d62e7611a180f"
